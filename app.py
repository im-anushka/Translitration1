...
def kn_remove_trailing_a(token: str) -> str:
    if token.endswith("ಅ") and len(token) > 1: 
        return token[:-1]
    return token
...
def process_KtoE(name: str) -> tuple[list, list]:
    ...
    for token in tokens:
        ...
        if not kn_is_initial(token_trans):
            token_trans = kn_remove_trailing_a(token_trans)
            others.append(token_trans)
        else:
            initials.append(token_trans)
...
def process_EtoK(name: str) -> tuple[list, list]:
    ...
    for token in tokens:
        ...
        if not is_initial(token_trans):
            token_trans = remove_trailing_a(token_trans)
            others.append(token_trans)
        else:
            initials.append(token_trans)
    return initials, others
...
def match_names_EC(name1: str, name2: str) -> tuple[bool, float, str]:
    ...
    tokens1 = [t.strip().lower() for t in tokens1]
    tokens2 = [t.strip().lower() for t in tokens2]
    ...
    # if tokens1 == tokens2:
    #     explanation += "\nTokens match exactly after normalization. Declaring as matched."
    #     return True, 1.0, explanation

    joined1 = " ".join(tokens1).lower()
    joined2 = " ".join(tokens2).lower()
    similarity = compute_similarity(joined1, joined2)  # This will now be case-insensitive
    ...
    if similarity > 0.75 and joined1 != joined2:
        no_vowel_joined1 = remove_vowels(joined1)
        no_vowel_joined2 = remove_vowels(joined2)
        new_similarity = compute_similarity(no_vowel_joined1, no_vowel_joined2)


        if new_similarity == 1.0:  
            explanation += "\nMatch found after removing vowels. Declaring as matched."
            return True, new_similarity, explanation
...
def match_names_KC(name1: str, name2: str) -> tuple[bool, float, str]:
    ...
    joined1 = " ".join(tokens1).lower()  # Convert to lowercase
    joined2 = " ".join(tokens2).lower()  # Convert to lowercase
    similarity = compute_similarity(joined1, joined2)  # This will now be case-insensitive
    threshold = 0.95  # Primary threshold
    if similarity < 0.75:
        # Fallback to Google Translate
